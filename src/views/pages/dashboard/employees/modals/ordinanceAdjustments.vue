            ...
            <v-dialog v-model="newOrdinanceAdjustmentModal" width="500">
                <v-card class="pa-5">
                    <v-form @submit.prevent="create" v-model="valid">
                        <v-col cols="12" >
                            <v-text-field name="name" label="Nombre" density="compact" required v-model="newOrdinanceAdjustment.name"
                                :rules="validations.titleRule('Nombre')" variant="outlined">
                            </v-text-field>
                        </v-col>

                        <v-col cols="12" >
                            <v-text-field name="amount" label="Monto" density="compact" prepend-inner-icon="mdi-currency-usd" required v-model="newOrdinanceAdjustment.amount"
            ...
            <v-data-table :headers="headers" :items="employeeStore.employee.ordinanceAdjustments">
                <template v-slot:item.actions="{ item }">
                    ...
                    <v-btn icon density="compact" color="red-lighten-1"><v-icon size="15"
                            @Click="() => { deleteDialogStore.openDialog(item.id, 'Ajuste por Ordenanza') }">mdi-trash-can</v-icon></v-btn>
                </template>
